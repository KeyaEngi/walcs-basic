...
!********Runge-Kutta第2,3,4步砰击载荷计算 

subroutine SlamCase4(RungeK,smtf,NumT,time0,time1,Dis0,Dis1,MEsecLoad0,MEsecLoad1,t0SlamForce,MainForce )
  
  use ShipHullVar,only:Cog
  use Slamming
  use ArrayOperations
  use Constant,only:Pi,U0,NR
  implicit none
    integer(4)::RungeK
    real(8)::smtf
    integer(4)::NumT
    ...
    real(8),dimension(3)::InteractionRVel  !----交点处的相对运动速度
    real(8),dimension(3)::temSvel,temWvel  !---交点处船与波面的临时绝对速度
    real(8),dimension(3)::temNor
    ...
    real(8),dimension(SlamNumLine,3)::SlineForce  !----当个砰击剖线上的砰击压力积分(在平动坐标系下的三个方向的力)
    
    real(8),dimension(NR)::MainForce

    real(8),dimension(3)::tempx

    integer(4)::i,j,k,it,ii,jj,kk,iii,jjj,kkk
    real(8)::s,t,s1,s2,t2,t1,s3,t3
    real(8)::a1,a2,a3,a4
    ...
    !**********************************
    temNumIntC=SlamNumIntC    !----版剖面插值Ct节点数量

    temSlamHV=0.03     !----砰击发生速度判据
    temSlamCV=0.01     !----砰击结束速度判据
    ...
    !---在此处应该给出time0时刻的temSlamCase和temIniPenetration
    temSlamCase(:)=SlamCase(:)
    temIniPenetration(:)=SlamIniPenetration(:) 
    temIniRise(:)=SlamIniRise(:)
    ...
    do i=1,SlamNumLine
        ...
        !20230427
        temp0(1:3)=VectorL2G(temp0(1:3),MDis0(4:6,i) )   !---因旋转运动产生的线位移
    ...
    do it=1,NumT
      ...
      time=time0+SlamDeltT*real(it) 
      if(it==NumT) time=time1

      !Dis=Dis0+DtDis*(time-time0 )   !---time时刻，船体摇荡运动的位移，速度
      !Vel=DtDis
      !20230427
        do i=1,SlamNumLine
            do j=1,6
      ...
      do i=1,SlamNumLine
          ...
          if(temSlamCase(i)==0 ) then
              ...
              if( temSlamRelaP(i,1)==0.and.temSlamRelaP(i,2)==1 ) then
                    ...
                    !-------与波面的交点默认为典型位置瞬时节点位置
                    InteractionP(1:3)=temP0VerticalWP(i,1:3)
                    ...
                    !*********注意：此处可以计算出(oXZ平面的速度方向(平动坐标系下))
                    !               以此来确定SlamAngle(i)，一经确认，在本次砰击结束前
                    !               SlamAngle(i)不会再改变

                    !------当前给定了SlamAngle(i)，因此在该瞬时方向上合成速度
                    ! temAngle表示实时剖线 与x轴正向的夹角
              ...
              elseif( temSlamRelaP(i,2)==1 ) then
                  !----此种情况包含砰击发生的第二种情况
                  !----需要进一步判断
                  !----当前SlamAngle(i)=Pi/2.0是默认的，后续会调整
                  temAngle1=SlamAngle(i)   !---选预估一个夹角
          ...
          if(temSlamCase(i)==1.and.SlamLineCase(i)==1  ) then
              ...
              !-----分别计算出T-2.0Delt T-1.0Delt T T+1.0Edlt T+2.0Edlt
              !-----以上5个时刻的相对运动位移，并据此确定先对运动速度，相对运动加速度
              temRelaL=0.0;   
              temRelaV=0.0;
              do j=1,7
                  ...
                  !----------临时时间存储变量
                  timeDt=time+riteT*SlamDeltT*DeltRait
                  !----临时船体重心位移，速度存储
                  ...
                  !---计算t时刻剖线实时倾角
                  temAngle=SlamAngle(i)-MtemDis(5,i)
...
!!                  if(j==1) then
!!                      k=j+1;
!!                      temRelaL2(j)=1./35.*(9.*temRelaL(k-1)+17.*temRelaL(k)+9.*temRelaL(k+1))
!!                  elseif(j==5) then
!!                      k=j+1;
!!                      temRelaL2(j)=1./35.*(9.*temRelaL(k-1)+17.*temRelaL(k)+9.*temRelaL(k+1))
!!                  else
!!                      k=j+1;
!!                      temRelaL2(j)=1./35.*(-3.*temRelaL(k-2)+12.*temRelaL(k-1)+17.*temRelaL(k)+12.*temRelaL(k+1)-3.*temRelaL(k+2))
!!                  end if
!!              end do
...
!!              InpRelaA=0.0;
          ...
          !-----time时刻砰击整体载荷叠加
          TimeSlamForce(it,1:NR)=TimeSlamForce(it,1:NR)+temSlamForce(i,1:NR)

      end do
      !--------------至此，time时刻所有剖线上的砰击载荷给定完毕
    ...
    do it=1,NumT
        ...
        MainForce(j)=MainForce(j)+(t1+t2)/2.0*SlamDeltT
