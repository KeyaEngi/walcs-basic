  ...
  real(8),dimension(1:3)::vel
	...
	Do j=1,Nl
        if (j<Nl) then
          ...
          eur(1,:,:)=Ur(i,j,:,:);       eur(2,:,:)=Ur(i+1,j,:,:);
          eur(3,:,:)=Ur(i+1,j+1,:,:);   eur(4,:,:)=Ur(i,j+1,:,:);
        else
          x1=Node(i,j,1);y1=Node(i,j,2);z1=Node(i,j,3)
          x2=Node(i+1,j,1);y2=Node(i+1,j,2);z2=Node(i+1,j,3)
	   ...
	   Do j1=1,2
		...
		ph4=(1.+cor2(j1))*(1.-cor2(jj))/4.
          ...
          !----节点各模态位移积分点
          tempur(:,:)=ph1*eur(1,:,:)+ph2*eur(2,:,:)+ph3*eur(3,:,:)+ph4*eur(4,:,:)

          !-----此处与FNL有出入,正负号问题(注意核查)
          vnaq(1)=-hs_x;
          vnaq(2)=-hs_y;
          vnaq(3)=-hs_z;
          vel(1)=xq-cog(1); vel(2)=yq-cog(2); vel(3)=zq-cog(3);
